#!/usr/bin/env python3
"""
Mininet + FRR OSPF benchmark script
Recreated from the SEED IE benchmark structure, but adapted for Mininet routers
running FRR inside Mininet namespaces instead of Docker containers.

What it measures
- latency, jitter, latency min/avg/max/std/median
- throughput and bandwidth via iperf3
- packet loss
- CPU and memory utilisation of the host
- estimated power/energy
- failure and recovery convergence

Outputs
- steady-state JSON + CSV
- convergence JSON + CSV
"""
...
DEFAULT_RESULTS_FILE = "mininet_frr_ospf_benchmark_results_iterative_ms.json"
CONV_CSV = "mininet_frr_ospf_benchmark_results_iterative_ms_convergence.csv"
CONV_JSON = "mininet_frr_ospf_benchmark_results_iterative_ms_convergence.json"
...
# Execution mode
# =========================
# "auto"   -> try Mininet process namespace first, then ip netns
# "mnexec" -> force Mininet process namespace
# "ipnetns"-> force ip netns exec <node>
EXEC_MODE = "auto"

# =========================
# Topology pairs
# Adjust IPs if your Mininet addressing differs
# =========================
LATENCY_PAIRS = [
...
# IMPORTANT: change FAIL_IFACE if needed
# =========================
CONVERGENCE_NODE = "r3"
CONVERGENCE_DST_IP = "10.0.56.2"
FAIL_IFACE = "r3-eth2"   # adjust to the interface you want to fail
...
def _resolve_mininet_pid(node: str) -> Optional[str]:
    """
    Try common Mininet process-name patterns.
    """
    patterns = [
        f"mininet:{node}$",
        f"mininet:{node} ",
...
def _build_node_exec_prefix(node: str) -> str:
    """
    Returns the prefix needed to execute a command inside a Mininet router namespace.
    """
    if EXEC_MODE in ("auto", "mnexec"):
        ...
        # Fallback: try locating Mininet node PID from /var/run/mininet
    ...
    if EXEC_MODE in ("auto", "ipnetns"):
        # This only works if namespaces are exposed by name
        try:
    ...
    raise RuntimeError(
        f"Could not resolve execution namespace for node '{node}'. "
        f"Set EXEC_MODE correctly or ensure the node process/namespace is visible."
    )


def node_exec(node: str, command: str, timeout: int = CMD_TIMEOUT) -> str:
    safe_command = command.replace('"', '\\"')
    prefix = _build_node_exec_prefix(node)
    return run_cmd(f'{prefix} sh -lc "{safe_command}"', timeout=timeout)
...
def estimate_energy_utilisation(cpu_busy_pct: float, duration_ms: float,
                                idle_w: float = ENERGY_IDLE_W,
                                max_w: float = ENERGY_MAX_W) -> Dict[str, float]:
...
# =========================
def run_ping_metrics(src_node: str, dst_ip: str, count: int, interval: float,
                     timeout: int = CMD_TIMEOUT) -> Dict[str, float]:
    output = node_exec(
        src_node,
    ...
    if not parsed:
        raise RuntimeError(f"Could not parse ping output for {src_node} -> {dst_ip}")
    return parsed


def run_iperf_metrics(src_node: str, dst_node: str, dst_ip: str,
                      duration: int = IPERF_DURATION) -> Dict[str, float]:
    try:
        node_exec(dst_node, "pkill -f iperf3 || true", timeout=15)
        node_exec(src_node, "pkill -f iperf3 || true", timeout=15)
    except Exception:
        pass

    # Ensure iperf3 exists
    for n in (src_node, dst_node):
        try:
            node_exec(n, "command -v iperf3 >/dev/null 2>&1", timeout=10)
        except Exception:
            raise RuntimeError(f"iperf3 is not installed or not in PATH inside node {n}")

    node_exec(dst_node, "nohup iperf3 -s >/tmp/iperf3_server.log 2>&1 &", timeout=15)
    time.sleep(1.0)

    try:
        output = node_exec(src_node, f"iperf3 -c {dst_ip} -t {duration} -J", timeout=duration + 30)
...
# Benchmark runs
# =========================
def run_steady_state_benchmark(iterations: int = DEFAULT_ITERATIONS,
                               pause_s: float = DEFAULT_PAUSE_BETWEEN_RUNS,
                               output_file: str = DEFAULT_RESULTS_FILE) -> None:
    raw_runs = []
    csv_rows = []
    ...
    for i in range(1, iterations + 1):
        ...
        for pair in LATENCY_PAIRS:
            try:
                res = run_ping_metrics(pair["src_node"], pair["dst_ip"], count=PING_COUNT, interval=PING_INTERVAL)
                res["hops"] = pair["hops"]
        ...
        for pair in BW_TPUT_PAIRS:
            try:
                res = run_iperf_metrics(pair["src_node"], pair["dst_node"], pair["dst_ip"], duration=IPERF_DURATION)
                res["hops"] = pair["hops"]
        ...
        for pair in LOSS_PAIRS:
            try:
                res = run_ping_metrics(pair["src_node"], pair["dst_ip"], count=PACKET_LOSS_COUNT, interval=PING_INTERVAL)
                loss_record = {
                    "hops": pair["hops"],
...
def preflight_checks() -> None:
    for binary in ("ping", "iperf3", "mnexec", "pgrep"):
